...
A cláusula 3.4 do contrato assinado é hoje texto digitado à mão a partir do laudo. Com `compatibilidades_coliving` populada, `server/legaldesign/mesclarTemplate.ts` ganha as variáveis `{{compatibilidade.scoreGeral}}`, `{{compatibilidade.resumoPontosAtrito}}` (gerado por uma função que transforma a lista estruturada em uma frase, ex.: *"divergência relevante em limpeza das áreas comuns"*) para o modelo de contrato de coliving — a cláusula passa a ser preenchida automaticamente a partir do dado já decidido pela gestão, não redigitada.

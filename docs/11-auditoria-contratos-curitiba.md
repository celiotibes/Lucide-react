...
## Sétimo achado: o texto corrido confirma a estrutura, mas revela uma heterogeneidade maior do que a estrutural

Você reenviou os três contratos originais, desta vez processados por completo (antes só os dados estruturais tinham sido extraídos para o schema). O texto corrido não muda nenhuma tabela — confirma cada componente já modelado — mas expõe que **a multa rescisória e a forma de pagamento aceita são diferentes em cada um dos três**, não só o valor:

| | Sala Comercial 923B | Apto 503 Central Station | Life Space 509B |
|---|---|---|---|
| Multa por saída antes de 12 meses | Fixa: 3 meses de aluguel | Fixa: **3 salários mínimos** (não aluguéis) | Proporcional: 50% dos meses restantes até completar 12, teto de 3 "aluguéis" (pacote aluguel+comodato) |
| Multa por infração genérica | Não especificada além da rescisória | 3 aluguéis vigentes | 3 aluguéis vigentes |
| Saída após 12 meses | Livre, sem multa, com 60 dias de aviso | Livre, sem multa, com 60 dias de aviso | Renovação automática por mais 12 meses; sem cláusula de saída livre depois de completado o primeiro ano — só reaviso 30 dias antes do fim de cada ciclo |
| Forma de pagamento do aluguel | Boleto, com **exceção expressa permitindo PIX** se o boleto não chegar a tempo | Boleto obrigatório; PIX só **excepcionalmente**, para o CPF do locador, se o boleto não chegar até a véspera do vencimento | Boleto **exclusivo** — cláusula veda expressamente PIX/depósito para o aluguel, mesmo como exceção |
| Forma de pagamento da caução | — (caução única de 3 aluguéis, sem detalhar meio) | Boleto ou PIX | **PIX obrigatório**, direto para o CPF do locador, em até 24h |

Isso não é um erro de leitura anterior — é a mesma heterogeneidade que você já tinha avisado ("os contratos de Curitiba são diferentes, com realidade diferente") aparecendo agora na camada de cláusula, não só na de componente financeiro. `contrato_politica_cobranca` (por contrato, não constante do sistema) já é a modelagem certa para a multa — nenhuma mudança de schema necessária. A forma de pagamento aceita não tem campo próprio ainda em lugar nenhum do schema (nem para Florianópolis) — fica registrado aqui como algo a decidir se/quando o pagamento por PIX avulso via Asaas for implementado de fato (hoje `emitirCobranca.ts` só emite boleto/PIX pelo Asaas sem essa distinção contratual).

**Outros dois achados novos do texto corrido, nenhum deles exige mudança de schema agora**:
- **Garantia de 7 dias contra defeito** (Life Space, Cláusula 4 Item C): qualquer item do imóvel com defeito nos primeiros 7 dias é reposto/consertado sem custo ao locatário — mesmo prazo de 7 dias que o Anexo II de Florianópolis usa para impugnação de vistoria (docs/10), mas com propósito diferente (ali é prazo para contestar o estado registrado; aqui é garantia ativa contra defeito). Coincidência de prazo, não de mecanismo — não generalizar os dois como a mesma regra.
- **Negativação/SPC após 5 dias, protesto em cartório** (Apto 503, Cláusula 12): reforça o mesmo gap já registrado como pendente em `docs/10` ("O que fica deliberadamente fora desta rodada") — `processos_judiciais`/`dossies_inadimplencia` existem, mas nenhuma tabela rastreia negativação/protesto como evento próprio antes da via judicial. Agora com 2 contratos reais (Floripa e Curitiba) confirmando que é uma prática recorrente, não uma cláusula isolada — sobe de prioridade para quando o Módulo 14 (Jurídico) avançar.
- **Taxa de emissão de boleto R$ 8,00**, presente nos três contratos de Curitiba de forma idêntica, somada ao valor cobrado — pequeno mas consistente o suficiente (3/3) para não ser coincidência; fica como nota, sem virar constante de schema por enquanto (valor pequeno, mudaria com o banco emissor).

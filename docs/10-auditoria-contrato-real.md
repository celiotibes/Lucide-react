...
## Sétimo achado: Anexo I chegou — regulamento interno, taxas e a lavanderia, com um conflito para decidir

Você enviou os "Anexos Administrativos" completos deste mesmo contrato (Anexo I a VI, 13 páginas, mesma kitnet 16). Isso resolve a maior parte do que a seção anterior deste documento listava como pendente:

- **Bonificação decrescente de dezembro (Anexo I, item 6)**: confirmado — desconto **fixo de 85%** sobre a multa rescisória proporcional (apurada pelo teto de 3 meses da cláusula 11.2) se o locatário notificar por e-mail **até 22/11** e entregar as chaves até 22/12; desconto **fixo de 80%** se notificar **até 27/11** e entregar até 28/12. Fora dessas janelas, não há bonificação — vale só o valor cheio do item 11.2. Isso fecha a lacuna registrada abaixo desde a rodada anterior; falta só implementar a fórmula (11.2 + esta régua de desconto) em código.
- **Taxa de declaração de residência (Anexo I, item 4)**: confirmado — **1 via grátis por semestre civil**. Vias adicionais no mesmo semestre, ou qualquer via que exija certificação digital ICP-Brasil ou remessa física, custam **R$ 30,00 por documento emitido**.
- **Pacotes extras de lavanderia (Manual do Inquilino item 6 + Anexo V, Parágrafo Terceiro)**: franquia grátis por tipo de moradia (kitnet 1 quarto = 1 ciclo roupa branca + 1 colorida/semana; apto 2 quartos = 2+2; apto 3 quartos = 3+3) — mas o Anexo V também limita por **morador autorizado** a até 2 ciclos/semana, então a franquia real é o menor entre os dois critérios quando há mais de 1 morador na kitnet. Acima da franquia, pacotes contratados por WhatsApp e cobrados no boleto: 2 ciclos extras R$ 25,00; 4 ciclos R$ 40,00; 6 ciclos R$ 55,00; 10 ciclos R$ 75,00 — valores sujeitos a reajuste por IPCA em renovações. Uso por terceiro não morador ou de itens proibidos (edredons grossos, tênis, itens de pet) gera multa de 10% do Aluguel Efetivo.
- **Água como comprovante de endereço**: o Anexo V (Parágrafo Primeiro) confirma explicitamente "a inexistência de hidrômetros de medição individualizada faturados por unidade autônoma (kitnet)" — reforça o achado da cláusula Nona já registrado acima. Não existe conta de água individual por kitnet neste modelo, então uma "fatura de água como comprovante de endereço" não tem de onde sair para Florianópolis do jeito que foi pedido; se a intenção era outra (ex.: uma declaração administrativa dizendo que o inquilino reside no endereço, e não uma fatura de concessionária), isso é o mesmo documento do item acima, não um novo.
- **Documento do locador (Anexo VI, item 1.1)**: os dados cadastrais completos de ambos os locadores (Célio Ribas Matzenbacher Tibes — CPF, RG, OAB/SC 057187; Avani Elvira Ribas Matzenbacher Tibes — CPF, RG) já constam no próprio contrato assinado. Para "anexar automaticamente o documento do locador" a uma declaração, não precisa de nada dinâmico — é um PDF estático (RG/CPF) por locador, guardado uma vez e referenciado por `pessoa_id` em todo documento gerado.
- **Fórmula de energia (Anexo IV)**: confirma, cláusula por cláusula, o que `calcularFaturaEnergia.ts` já implementa — leitura por submedidor no dia 1º, tarifa CELESC vigente, bandeiras ANEEL, **repasse exato, sem acréscimo/margem/taxa de administração** sobre o kWh do inquilino. Nenhuma mudança de código necessária; serve como segunda confirmação real, independente do contrato original que já validou a mesma fórmula.

**Conflito identificado e já resolvido por você**: o Anexo III, item B.2 ("Câmeras e Privacidade") diz que é **"vedada a solicitação de imagens por inquilinos para fins particulares, exceto sob ordem judicial ou policial."** Você confirmou a resposta: o pedido do inquilino não é aprovado pelo admin — vira **análise do jurídico** para verificar se há alguma exceção legal aplicável, mantendo a regra do anexo como padrão (ou seja, negar é o default; só uma exceção legal fundamentada muda isso). Implementado em `server/integracao/solicitarImagensCameras.ts` e `solicitacoes_imagens_cameras` (schema seção 26, `docs/26-solicitacoes-especializadas.md`).

**Esclarecimento sobre "fatura de água como comprovante"**: não é sobre o inquilino provar residência — é sobre o **proprietário** provar o vínculo com o imóvel (conta de água em nome dele, no endereço do imóvel) para não precisar enviar a matrícula, documento mais caro/burocrático de obter. Fica registrado como pendência de cadastro de documentação do proprietário (campo em `imoveis`/`pessoas`), não como funcionalidade do portal do inquilino — ainda sem código associado.

**Dois pedidos novos, também a partir dos anexos**: solicitação de chave reserva/cópia, com valor lido de `contrato_tarifario_servicos` (generalização da "Planilha de Taxas de Manutenção" do Anexo II, item 2 — chaveiro abertura/perda R$ 140,00 — por contrato, não constante fixa do sistema); e pedido administrativo de autorização para internet particular, com orientação de instalação cadastrável por imóvel (`imoveis.orientacoes_instalacao_internet`) — nunca inventada por código, porque é dado físico de cada prédio. Ambos implementados em `docs/26-solicitacoes-especializadas.md`.

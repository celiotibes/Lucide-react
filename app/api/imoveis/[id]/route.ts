...
export async function DELETE(
  ...
  try {
    ...
    // Check if property has active contracts or other dependencies
    const { rows: contratoRows } = await pool.query(
      `select count(*) as count from contratos where imovel_id = $1 and status != 'encerrado'`,
      [id]
    );

    if (contratoRows[0].count > 0) {
      return NextResponse.json(
        { erro: 'Não é possível deletar imóvel com contratos ativos' },
        { status: 400 }
      );
    }

    // Check for comodos with active contracts
    const { rows: comodosRows } = await pool.query(
      `select count(*) as count from comodos c
       where c.imovel_id = $1 and c.ativo = true`,
      [id]
    );

    if (comodosRows[0].count > 0) {
      return NextResponse.json(
        { erro: 'Não é possível deletar imóvel com cômodos ativos' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ sucesso: true, mensagem: 'Imóvel deletado com sucesso' });
  } catch (erro) {
    console.error('Erro ao deletar imóvel:', erro);
    return NextResponse.json({ erro: 'Erro ao deletar imóvel' }, { status: 500 });
  }
}

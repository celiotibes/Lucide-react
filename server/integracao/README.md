...
Motor de geração de contrato em HTML/CSS (Legal Design/Visual Law, `docs/27-motor-de-contratos.md`). Funde o modelo regional ativo da cidade do imóvel (`modelos_contrato`) com os dados do contrato: partes (`contrato_partes`/`pessoas`), garantia (`garantias`), e o inventário de mobília (`ativos_comodato`, reaproveitado do módulo de patrimônio/CPC 27) — se o contrato tem `comodo_id` (co-living por quarto), a mobília fica restrita ao cômodo + áreas comuns; senão, é o inventário inteiro do imóvel. O merge em si é `../legaldesign/mesclarTemplate.ts` (substituição escapada de `{{variavel}}` e bloco `{{#each}}` — sem Handlebars como dependência nova). Só devolve a string HTML — não persiste em `documentos_gerados`/storage ainda, porque isso depende do Supabase Storage (bloqueado por credencial, `docs/09`).

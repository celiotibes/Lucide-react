...
Auditoria de geração solar com créditos de compensação (`docs/30-auditoria-geracao-solar.md`) — cruza `geracao_solar` confirmada, `faturas_celesc_gd` confirmada (lançamento manual — sem fatura real de GD em mãos, um parser seria adivinhar layout) e o consumo já cobrado dos inquilinos (mesmo pareamento leitura atual/anterior de `faturarEnergia.ts`, agregado por residencial) para isolar o consumo de área comum via `../energia/auditoriaGeracaoSolar.ts` (função pura). Só calcula quando as duas fontes humanas já foram confirmadas — motivo explícito (`sem_geracao_confirmada`, `sem_fatura_celesc_confirmada`, `sem_leitura_de_inquilino_no_periodo`) quando não dá. Cliente da API do ShinePhone/Growatt (`server/growatt/client.ts`) ainda não existe — depende de credencial.

// Lógica de aprovação/rejeição de uma extração de dados de contrato feita
// pela IA, separada da action 'use server'
// (app/contratos/[id]/documentos/[documentoId]/revisao/actions.ts) para ser
// testável — mesmo padrão de decidirReajuste.ts.
//
// Só os campos que o operador marcou explicitamente no formulário são
// copiados de extracoes_documento_ia.dados_extraidos para
...
//
// Quando um custo obrigatório é marcado mas o componente já cadastrado tem
// natureza incompatível (não é 'valor_fixo'), a aplicação é pulada — antes
// isso acontecia em silêncio; agora o motivo é registrado em
// `campos_aplicados.custosNaoAplicados` para a tela de revisão poder avisar
// o operador.

import type { Pool } from 'pg';
import type { DadosContratoExtraidos } from '@/server/documentos/extrairDadosContrato';

export interface ResultadoDecisaoExtracao {
  sucesso: boolean;
  erro?: string;
}

export async function rejeitarExtracao(pool: Pool, extracaoId: string): Promise<ResultadoDecisaoExtracao> {
  const { rowCount } = await pool.query(
    `update extracoes_documento_ia set status = 'rejeitada', revisado_por = $1, revisado_em = now()
     where id = $2 and status = 'pendente_revisao'`,
    [null, extracaoId],
  );
  if (!rowCount) {
    return { sucesso: false, erro: 'Extração não encontrada ou já revisada' };
  }
  return { sucesso: true };
}
...
export async function aprovarExtracao(
  ...
  contratoId: string,
  formData: FormData,
): Promise<ResultadoDecisaoExtracao> {
  ...
  if (!extracao || !extracao.dados_extraidos) {
    return { sucesso: false, erro: 'Extração não encontrada ou sem dados' };
  }
  if (extracao.status !== 'pendente_revisao') {
    return { sucesso: false, erro: 'Esta extração já foi revisada' };
  }
  ...
  try {
    ...
    const custosNaoAplicados: { tipo: string; motivo: string }[] = [];
    for (const custo of dados.custosObrigatorios ?? []) {
      ...
      if (componenteExistente[0]) {
        ...
        if (componenteExistente[0].natureza === 'valor_fixo') {
          ...
          custosNaoAplicados.push({
            tipo: custo.tipo,
            motivo: `já existe um componente de natureza "${componenteExistente[0].natureza}" cadastrado para este tipo — não é seguro sobrescrever com o valor fixo extraído do documento`,
          });
        }
      } else {
        await client.query(
    ...
    if (custosNaoAplicados.length > 0) {
      camposAplicados.custosNaoAplicados = custosNaoAplicados;
    }
    ...
    return { sucesso: false, erro: erro instanceof Error ? erro.message : 'Erro ao aprovar extração' };
  } finally {
    client.release();
  }
}

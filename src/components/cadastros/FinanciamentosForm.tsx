...
function formVazio(imovelPadrao: number | null): Formulario {
  return { id: null, imovel_id: imovelPadrao, instituicao: "", sistema: "SAC", valor_contratado: "", taxa_juros_mensal: "0,8", data_contrato: "", parcelas_total: "360", observacoes: "" };
}
...
export function FinanciamentosForm() {
  ...
  function abrirEdicao(f: Financiamento) {
    setForm({
      ...
      data_contrato: f.data_contrato, parcelas_total: f.parcelas_total.toString(), observacoes: f.observacoes ?? "",
    });
  }
  ...
  async function salvar() {
    ...
    if (form.id === null) {
      executar(
        db,
        "INSERT INTO financiamentos (imovel_id, instituicao, sistema, valor_contratado, taxa_juros_mensal, data_contrato, parcelas_total, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [form.imovel_id, form.instituicao.trim(), form.sistema, valorContratado, taxaJuros, form.data_contrato, parcelas, form.observacoes.trim() || null],
      );
    } else {
      executar(
        db,
        "UPDATE financiamentos SET imovel_id = ?, instituicao = ?, sistema = ?, valor_contratado = ?, taxa_juros_mensal = ?, data_contrato = ?, parcelas_total = ?, observacoes = ? WHERE id = ?",
        [form.imovel_id, form.instituicao.trim(), form.sistema, valorContratado, taxaJuros, form.data_contrato, parcelas, form.observacoes.trim() || null, form.id],
      );
    }
    await persistir();
    setForm(null);

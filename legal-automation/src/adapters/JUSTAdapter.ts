...
export class JUSTAdapter implements TribunalAdapter {
  private baseUrl: string;
  private httpClient: any;

  constructor() {
    this.baseUrl = config.just_api_url || 'https://api.datajud.cnj.jus.br/api/v1';
    ...
    if (config.just_api_key) {
      this.httpClient.defaults.headers.common['Authorization'] = `Bearer ${config.just_api_key}`;
    }
  }
  ...
  async getProcess(number: string): Promise<Process> {
    try {
      logger.debug(`JUST: Obtendo processo ${number}`);
      ...
      logger.error({ err: error }, `JUST: Erro ao obter processo ${number}`);
      throw error;
    }
  }
  ...
  async searchProcesses(criteria: SearchCriteria): Promise<Process[]> {
    try {
      logger.debug(`JUST: Buscando processos com critérios:`, criteria);

      // JUST uses unified DataJud API
      const processes = await dataJudClient.searchProcesses({
      ...
      logger.error({ err: error }, 'JUST: Erro ao buscar processos');
      throw error;
    }
  }
  ...
  async getMovements(processNumber: string): Promise<Movement[]> {
    try {
      logger.debug(`JUST: Obtendo movimentações do processo ${processNumber}`);
      ...
      logger.error({ err: error }, `JUST: Erro ao obter movimentações ${processNumber}`);
      throw error;
    }
  }
  ...
  async submitPetition(
    ...
    try {
      logger.debug(`JUST: Enviando petição para processo ${petition.processNumber}`);

      // JUST (PDPJ-Br) unified system
      ...
      logger.info(`JUST: Petição enviada com protocolo ${response.data.protocolo}`);
      ...
      logger.error({ err: error }, 'JUST: Erro ao enviar petição');
      return {
        protocolo: '',
  ...
  async getPetitionStatus(protocolNumber: string): Promise<PetitionStatus> {
    try {
      logger.debug(`JUST: Consultando status do protocolo ${protocolNumber}`);
      ...
      logger.error({ err: error }, `JUST: Erro ao consultar protocolo ${protocolNumber}`);
      throw error;
    }
  }
  ...
  async validateCertificate(cert: Buffer, password: string): Promise<boolean> {
    try {
      logger.debug('JUST: Validando certificado');

      if (!cert || cert.length === 0) {
        return false;
      }

      // JUST accepts various certificate types
      const header = cert.toString('hex', 0, 4);
      return header === '3082' || header === '3083'; // PFX magic bytes
    } catch (error) {
      logger.error({ err: error }, 'JUST: Erro ao validar certificado');
      return false;
    }
  }
...
export const justAdapter = new JUSTAdapter();
